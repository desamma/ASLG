...
public class Enemy_Chakkram_Movement : MonoBehaviour, IEnemy_Movement, IEnemyMovementContext
{
    ...
    private StateManager<Enemy_Chakkram_State> stateManager;

    [Header("Components")]
    [SerializeField] private Rigidbody2D rb;
    [SerializeField] private Animator animator;
    [SerializeField] private LayerMask playerLayer;
    private List<AttackCategory<Enemy_Chakkram_State>> attackCategories;
    ...
    private EnemyMoveCooldownTracker<Enemy_Chakkram_State> moveCooldowns = new();
    ...
    private void RegisterMoveUsed(Enemy_Chakkram_State usedState)
    {
    ...
    {
        if (!stateManager.IsInState(Enemy_Chakkram_State.Chase))
            stateManager.ChangeState(Enemy_Chakkram_State.Chase);
    }

    public void ChangeToIdleState()
    {
        if (!stateManager.IsInState(Enemy_Chakkram_State.Idle))
            stateManager.ChangeState(Enemy_Chakkram_State.Idle);
    }

    public bool IsInAnyAttackState()
    {
        return stateManager.IsInState(Enemy_Chakkram_State.Attack);
    }
    ...
    private void Start()
    {
        ...
        stateManager = new StateManager<Enemy_Chakkram_State>(animator, Enemy_Chakkram_State.Idle);
    ...
    private void Update()
    {
        ...
        if (!stateManager.IsInState(Enemy_Chakkram_State.Knockback) && !IsRecovering)
        {
            CheckForPlayer();
        }
        ...
        else if (stateManager.IsInState(Enemy_Chakkram_State.Patrol))
            Patrol();
    }
    ...
    private void Patrol()
    {
        EnemyMovementHelper.Patrol(this, patrolPoints, ref currentPatrolIndex, ref isWaiting, ref waitTimer,
            ...
            () => stateManager.ChangeState(Enemy_Chakkram_State.Idle),
            () => stateManager.ChangeState(Enemy_Chakkram_State.Patrol));
    }
    ...
    public void CheckForPlayer()
    {
        ...
        {
            if (!stateManager.IsInState(Enemy_Chakkram_State.Patrol) &&
                !IsInAnyAttackState())
            {
                stateManager.ChangeState(Enemy_Chakkram_State.Patrol);
            }
    ...
    {
        attackCategories = new List<AttackCategory<Enemy_Chakkram_State>>
        {
            new() {
                Frequency = 1f,
                Attacks = new[]
                {
                    ...
                    new AttackConfig<Enemy_Chakkram_State> { State = Enemy_Chakkram_State.Attack, Range = Stats.AttackRange, MoveCountCooldown = 0 }
                }
            }
        };
    ...
    private void OnStateEnter(Enemy_Chakkram_State state)
    {
        switch (state)
        {
            case Enemy_Chakkram_State.Attack:
                rb.velocity = Vector2.zero;
                isKnockbackable = false;
                FacingDirection = TransformHelper.FlipTowards(transform, PlayerTransform, FacingDirection);
                break;
            case Enemy_Chakkram_State.Death:
                rb.velocity = Vector2.zero;
    ...
    private void OnStateExit(Enemy_Chakkram_State state)
    {
        switch (state)
        {
            case Enemy_Chakkram_State.Attack:
    ...
    public void KnockBack(Transform player, float knockbackForce, float knockbackTime, float stunTime)
    {
        ...
        knockbackHandler.ApplyKnockback(transform, player, knockbackForce, knockbackTime, stunTime,
            () => stateManager.ChangeState(Enemy_Chakkram_State.Knockback),
            () =>
            {
                stateManager.ChangeState(Enemy_Chakkram_State.Idle);
            });
    ...
    #region Getters
    public StateManager<Enemy_Chakkram_State> GetStateManager() => stateManager;
